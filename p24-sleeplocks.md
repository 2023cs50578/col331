...
time. For this, we introduce a lock with each buffer in `bio.h`. However, a
process can hold a buffer for a long time; reading/writing a buffer can sleep in
`ide.c`.  Therefore, it does not make sense to disable interrupts for the long
time during which a process is working with a buffer.
...
process just sleep on the lock. It gets woken up when the sleep lock is released.
